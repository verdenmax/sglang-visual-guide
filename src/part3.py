...
<div class="fig">
  ...
  <div class="figcap"><b>图 1 · 解释器 vs 追踪器</b> — 解释器把程序当成"立刻照办的指令"，<strong>逐节点 eager 执行</strong>、遇 gen 即调后端；追踪器把它当成"待研究的图纸"，<strong>先静态展开成图</strong>再让<strong>独立分支并行</strong>，交给运行时优化。</div>
</div>
...
<div class="fig">
  ...
  <div class="figcap"><b>图 2 · 并行执行 fork 分支</b> — StreamExecutor 运行时，<span class="mono">fork</span> 把数据流分成 3 条<strong>并行车道</strong>，它们<strong>共享同一段父前缀</strong>、各自独立<strong>调用后端</strong>并发生成。</div>
</div>
...
<div class="fig">
  ...
  <div class="figcap"><b>Fig 1 · interpreter vs tracer</b> — the interpreter treats the program as "instructions to obey now," running it <strong>node by node, eagerly</strong> and calling the backend at each gen; the tracer treats it as "a blueprint to study," <strong>statically expanding it into a graph first</strong> so <strong>independent branches run in parallel</strong>, handing the runtime something to optimize.</div>
</div>
...
<div class="fig">
  ...
  <div class="figcap"><b>Fig 2 · executing forks in parallel</b> — while StreamExecutor runs, <span class="mono">fork</span> splits the stream into 3 <strong>parallel lanes</strong> that <strong>share the same parent prefix</strong> and each <strong>call the backend</strong> concurrently.</div>
</div>
...
发给我们将在<strong>第 13 课起</strong>逐层拆解的那台服务器——入口的 TokenizerManager（第 14 课）、大脑 Scheduler（第 18–23 课）、执行的 ModelRunner（第 24 课）。<strong>正因为请求落到了我们自己的运行时</strong>，
...
<div class="codefile">
  ...
  <pre><span class="kw">class</span> RuntimeEndpoint(BaseBackend):
    ...
    <span class="kw">def</span> generate(self, s, sampling_params):
        ...
        <span class="kw">return</span> res.json()[<span class="st">"text"</span>], res.json()[<span class="st">"meta_info"</span>]  <span class="cm"># 完成文本 + 元信息</span></pre>
</div>
...
sent to the server we dissect layer by layer <strong>from Lesson 13 onward</strong> — the TokenizerManager at the door (Lesson 14), the Scheduler as the brain (Lessons 18–23), the ModelRunner doing the forward (Lesson 24). <strong>Precisely because the request lands on our own runtime</strong>,
...
<div class="codefile">
  ...
  <pre><span class="kw">class</span> RuntimeEndpoint(BaseBackend):
    ...
    <span class="kw">def</span> generate(self, s, sampling_params):
        ...
        data = {<span class="st">"text"</span>: s.text_, <span class="st">"sampling_params"</span>: {...}}
        res = http_request(self.base_url + <span class="st">"/generate"</span>, json=data)
        <span class="kw">return</span> res.json()[<span class="st">"text"</span>], res.json()[<span class="st">"meta_info"</span>]  <span class="cm"># completion text + meta</span></pre>
</div>
