...
<div class="fig">
  <svg viewBox="0 0 800 320" role="img" aria-label="分页 KV 收集：连续的逻辑序列经 page_table 的 kv_indices 指向 KV 池里物理分散的页，核函数据此把正确的页收集起来再做注意力">
    ...
    <rect x="636" y="150" width="84" height="38" rx="6" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    ...
    <rect x="690" y="224" width="84" height="38" rx="6" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
  ...
  <div class="figcap"><b>图 1 · 分页 KV 收集</b> — 逻辑上连续的序列（<span class="mono">L0…L3</span>）经 <span class="mono">page_table / kv_indices</span>（同一"逻辑→物理"映射的两种布局）翻译成物理上分散的 KV 页（#7、#3、#19、#5）；核函数先按 <span class="mono">kv_indices</span> 把这些散落的页收集回来，才能把它们当作一条序列来注意。</div>
</div>
...
<div class="fig">
  <svg viewBox="0 0 800 300" role="img" aria-label="分块注意力内循环：一个 Q 瓦片流式遍历各 KV 瓦片，每块算 S 等于 Q 乘 K 转置，用在线 softmax 更新运行最大值与分母，再把乘 V 的结果累加进累加器，完整分数矩阵从不落 HBM">
    ...
    <rect x="278" y="90" width="92" height="46" rx="6" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    ...
    <rect x="166" y="172" width="452" height="66" rx="8" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
...
<div class="fig">
  ...
  <div class="figcap"><b>Fig 1 · Paged-KV gather</b> — a logically contiguous sequence (<span class="mono">L0…L3</span>) is translated through the <span class="mono">page_table / kv_indices</span> (two layouts of the same logical→physical map) into physically scattered KV pages (#7, #3, #19, #5); the kernel first gathers those scattered pages by <span class="mono">kv_indices</span> before it can attend to them as one sequence.</div>
</div>
...
<div class="fig">
  <svg viewBox="0 0 800 330" role="img" aria-label="一个 SGLang 引擎扇出到多种芯片，每种经平台抽象接入，用能力标志回答上层">
    ...
    <text x="670" y="164" text-anchor="middle" style="font-size:11px;fill:var(--muted)">hardware_backend</text>
    ...
    <text x="150" y="278" text-anchor="middle" style="font-size:11px;fill:var(--muted)">hardware_backend</text>
    ...
    <text x="410" y="278" text-anchor="middle" style="font-size:11px;fill:var(--muted)">hardware_backend</text>
...
<div class="fig">
  <svg viewBox="0 0 800 330" role="img" aria-label="One SGLang engine fans out to many chips, each integrated via the platform layer, answering capability flags">
    <rect x="300" y="20" width="200" height="48" rx="8" style="fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.5"/>
