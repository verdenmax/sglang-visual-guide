...
_PH = "PLACEHOLDER — to be authored by subagent. " * 4
_PH_EN = "PLACEHOLDER — to be authored by subagent. " * 3
...
<div class="card analogy"><div class="tag">🔌 打个比方 / Analogy</div>把 SGLang 主工程想象成一台组装好的整车,Python 代码是仪表盘和方向盘——你转动方向盘,车就转弯。而 <strong>sgl-kernel</strong> 是发动机厂:它在另一条生产线上,用完全不同的工艺(C++/CUDA、CMake)把发动机<strong>预先造好</strong>,装箱(打进 <span class="mono">.so</span>)后送到整车厂。你坐进驾驶室时,发动机早就在引擎盖下待命了,你只需踩油门(调用 <span class="mono">torch.ops.sgl_kernel.merge_state_v2</span>),根本不用关心活塞和缸体是怎么铸造的。仪表盘上每个按钮(Python 薄包装)背后,都接着一根线缆(torch 算子注册表)通向发动机里某个具体零件(<span class="mono">csrc/</span> 里的某个 kernel)。</div>

<div class="card macro"><div class="tag">🌍 放到全局 / The big picture</div>为什么要单独搞一个工程来放核函数?因为 GPU kernel 的编译与 Python 的运行节奏完全不同。Python 是解释执行、随改随跑;CUDA kernel 却要经过 <span class="mono">nvcc</span> 编译、链接,耗时几分钟。如果每次启动服务都现编译,延迟无法接受。所以 SGLang 选择把最常用、最稳定的核函数<strong>提前编译(Ahead-Of-Time, AOT)</strong>,塞进 wheel 里的 <span class="mono">.so</span>,装好即用。与之相对的是<strong>即时编译(JIT)</strong>——某些实验性或形状高度可变的 kernel 会在运行时再编(这是第39课的前瞻主题)。回想第4课讲过:解码阶段是<strong>带宽受限</strong>的,每生成一个 token 都要把权重和 KV 从显存搬一遍,核函数写得好不好,直接决定显存带宽用满了几成,也就直接决定吞吐。所以 sgl-kernel 不是锦上添花,而是整个推理引擎的性能地基。</div>
...
<div class="card key"><div class="tag">🎯 本课要点 / Key points</div><ul>
...
<div class="codefile"><div class="cf-head"><span class="dot"></span><span class="path">sgl-kernel/python/sgl_kernel/attention.py ::cutlass_mla_decode</span><span class="ln">按 page_table 收集分页 KV → 分块 Q·Kᵀ → 在线 softmax → ·V</span></div><pre>def cutlass_mla_decode(
    q_nope, q_pe,                 # query: compressed (nope) + rope parts
    kv_c_and_k_pe_cache,         # paged KV: latent (c) + key-rope, one entry per page
...
<p>注意 <span class="mono">forward_native</span> 里那行 <span class="mono">F.silu(x[..., :d]) * x[..., d:]</span>：它把张量从隐藏维的中点切成前后两半，前半是门控（gate）、后半是上投影（up）。表达式先算 silu，PyTorch 会为这个中间结果分配一块显存并写入，随后乘法再把它读回来——这就是那一次多余的临时张量往返。而 <span class="mono">forward_cuda</span> 先用 <span class="mono">torch.empty</span> 备好输出缓冲，再调用 <span class="mono">silu_and_mul(x, out)</span> 这个自定义算子，它最终落到 <span class="mono">torch.ops.sgl_kernel.silu_and_mul</span>，在 GPU 上以单趟方式同时完成切分、silu 与逐元素乘。输出缓冲尺寸只依赖于隐藏维的一半 <span class="mono">d</span>，是个静态值，因此这个内核在解码时形状固定、可以被安心地捕获进图反复重放。</p>
...
<div class="codefile"><div class="cf-head"><span class="dot"></span><span class="path">python/sglang/srt/layers/activation.py ::SiluAndMul</span><span class="ln">未融合：两趟 + 临时张量；融合：一个 kernel 同时做 silu 与乘</span></div><pre>class SiluAndMul(MultiPlatformOp):
    ...
    def forward_cuda(self, x):          # FUSED: one kernel does silu AND mul
        ...
        silu_and_mul(x, out)            # -&gt; torch.ops.sgl_kernel.silu_and_mul
        return out</pre></div>
