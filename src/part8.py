...
<p>实现里还有两个常被问到的细节。其一是<strong>词表对齐</strong>：词表大小要<strong>向上补齐</strong>到一个固定粒度（源码里是 <span class="mono">DEFAULT_VOCAB_PADDING_SIZE=64</span> 的倍数），必要时再补到能被 rank 数整除，<span class="mono">VocabParallelEmbedding</span> 据此把词表补齐，多出来的是"假 token"，它们的 logit 会被屏蔽掉、永远不会被选中，只为让每张卡分到等长的一段、对齐通信。其二是<strong>嵌入与输出头共享</strong>：当模型用 tied embedding 时，输入端的 <span class="mono">VocabParallelEmbedding</span> 和输出端的 <span class="mono">ParallelLMHead</span> 其实是<strong>同一份按词表切好的权重</strong>，输入时用它把 token id 查成向量、输出时用它把向量打成分数，省下一整张大表的显存。无论共享与否，切分的维度和通信的方式都一致。</p>
