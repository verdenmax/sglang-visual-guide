...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">layers/attention/base_attn_backend.py ::AttentionBackend</span><span class="ln">委托契约：规划元数据 + 两条前向路径</span></div>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">layers/attention/base_attn_backend.py ::AttentionBackend</span><span class="ln">the delegation contract: plan metadata + two forward paths</span></div>
  <pre><span class="kw">class</span> <span class="st">AttentionBackend</span>(ABC):
...
<p>关键的账要算清楚：假设有 64 个专家、每个 token 走 2 个，那么这一层的<strong>参数</strong>大约是稠密版的 32 倍（知识容量大涨），可每个 token 的<strong>计算量</strong>只相当于"过 2 个小 FFN"——
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">layers/moe/fused_moe_triton/layer.py ::FusedMoE</span><span class="ln">N 个专家 + 路由 top-k，融合进 kernel</span></div>
...
<p>Do the arithmetic: with 64 experts and 2 per token, this layer's <strong>parameters</strong> are roughly 32x a dense version (huge knowledge capacity), yet each token's <strong>compute</strong> is just "pass through 2 small FFNs" —
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">layers/moe/fused_moe_triton/layer.py ::FusedMoE</span><span class="ln">N experts + top-k routing, fused into kernels</span></div>
  <pre><span class="kw">class</span> <span class="st">FusedMoE</span>(torch.nn.Module):
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">layers/rotary_embedding/base.py ::RotaryEmbedding</span><span class="ln">前向：按位置把旋转施加到 q/k</span></div>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">layers/rotary_embedding/base.py ::RotaryEmbedding</span><span class="ln">forward: applying the rotation to q/k by position</span></div>
  <pre><span class="kw">def</span> forward_native(self, positions, query, key, ...):
...
<p>解码阶段这个优化更是天然成立：每步只往前走一个 token，每条请求的 hidden 本来就只有<strong>一行</strong>（最新那个位置），lm_head 自然只算一行。所以"末位切片"在预填充时是<strong>主动裁剪</strong>、在解码时是<strong>本就如此</strong>——两种模式（第 30 课）在这里被统一成同一句话：<strong>每条请求只取一行 hidden 去算 logits</strong>。也正因为输出头只处理这薄薄的几行，它在整个前向里的耗时占比通常很小；真正的大头还是中间那几十层注意力与 MLP。把这件事想清楚，你就能解释一个常见困惑：模型参数里词表头占了一大块<strong>显存</strong>，但它在每步解码里花的<strong>时间</strong>却很少——因为算的行数实在太少了。</p>
...
<div class="codefile">
  ...
  <pre><span class="kw">class</span> LogitsProcessor(nn.Module):
    <span class="kw">def</span> forward(self, input_ids, hidden_states,
        ...
        last_hidden = self._slice_last_token(hidden_states, logits_metadata)
        <span class="cm"># 2) 词表并行投影：本 rank 只算自己那段词表的 logits</span>
        logits = lm_head(last_hidden)               <span class="cm"># [tokens, vocab/这段]</span>
...
<p>In decode this optimization is automatic: each step advances one token, so each request's hidden is already just <strong>one row</strong> (the newest position) and lm_head naturally scores one row. So "last-token slice" is an <strong>active prune</strong> in prefill and <strong>already-the-case</strong> in decode — the two modes (Lesson 30) collapse to one sentence here: <strong>each request takes one row of hidden to compute logits</strong>. Precisely because the output head touches only these few rows, its share of forward time is usually small; the real cost is the dozens of attention and MLP layers in the middle. Grasp this and you can explain a common puzzle: the vocab head is a big chunk of <strong>HBM</strong>, yet spends little <strong>time</strong> per decode step — because it computes so few rows.</p>
...
<div class="codefile">
  ...
  <pre><span class="kw">class</span> LogitsProcessor(nn.Module):
    <span class="kw">def</span> forward(self, input_ids, hidden_states,
        ...
        logits = lm_head(last_hidden)               <span class="cm"># [tokens, vocab/segment]</span>
