...
<div class="card macro">
  ...
  决定每一步<strong>这一批由谁组成</strong>。它<strong>只决策、不计算</strong>——真正的 GPU 矩阵乘交给 TpModelWorker→ModelRunner（第 24 课）。
...
<div class="vflow">
  ...
  <div class="step"><div class="num">4</div><div class="sc"><h4>run_batch()——前向</h4><p>把这一批交给 TpModelWorker → <span class="mono">ModelRunner.forward</span> 上 GPU 跑一遍（第 24 课），拿到 logits 后<strong>采样</strong>出新 token。</p></div></div>
...
<p>把镜头再拉远一点接回第 2 课的<strong>三进程模型</strong>：TokenizerManager 在一个进程把文字变 token，Scheduler（连同 TpModelWorker）在中间这个进程做调度与前向，
...
<table class="t">
  ...
  <tr><td><strong>run_batch</strong></td><td>交 TpModelWorker→ModelRunner.forward 上 GPU，采样出 token</td><td class="mono">第 24 课 模型前向</td></tr>
...
<div class="cols">
  ...
  <div class="col"><h4>TpModelWorker / ModelRunner：计算但不决策</h4><p>拿到调度器组好的批，老老实实在 <strong>GPU</strong> 上跑
...
<p>第 4 步 <span class="inline">run_batch</span> 本身其实<strong>极薄</strong>——它把整批交给 TpModelWorker 做<strong>一次</strong>前向，拿回 logits 与采样好的 token，
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>决策 vs 计算</strong>：调度器单线程、唯一决策者，独占 KV 账本、只做 CPU 轻量记账；TpModelWorker/ModelRunner 才上 GPU 做 forward（第 24 课）。</li>
...
<div class="card macro">
  ...
  batch</strong>. It <strong>only decides, never computes</strong> — the real GPU matmuls go to TpModelWorker→ModelRunner (Lesson 24).
...
<div class="vflow">
  ...
  <div class="step"><div class="num">4</div><div class="sc"><h4>run_batch() — forward</h4><p>Hand the batch to TpModelWorker → <span class="mono">ModelRunner.forward</span> on the GPU (Lesson 24), get logits, then <strong>sample</strong> the new tokens.</p></div></div>
...
<div class="cols">
  ...
  <div class="col"><h4>TpModelWorker / ModelRunner: computes, doesn't decide</h4><p>It takes the batch the scheduler formed and dutifully runs
...
<p>Step 4, <span class="inline">run_batch</span>, is itself <strong>very thin</strong> — it hands the whole batch to the TpModelWorker for
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>Decide vs compute</strong>: the scheduler is single-threaded, the sole decision-maker, owns the KV ledger, does only light CPU accounting; only TpModelWorker/ModelRunner runs forward on the GPU (Lesson 24).</li>
