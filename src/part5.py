...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">schedule_policy.py ::SchedulePolicy.calc_priority</span><span class="ln">排序 + 限流</span></div>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">schedule_policy.py ::SchedulePolicy.calc_priority</span><span class="ln">order + throttle</span></div>
<pre><span class="kw">def</span> calc_priority(self, waiting_queue, running_batch=<span class="kw">None</span>):
...
<div class="card macro">
  ...
  代价是<strong>多一拍延迟</strong>（你总是慢一步）外加更精细的状态管理——下一批要在当前结果还没出来时就先搭好，SGLang 用 <span class="mono">batch_overlap</span> 那套
...
SGLang 用 <span class="mono">batch_overlap</span> 目录里的 future/event 对象替你把这些依赖串好、保证正确。还有些场景（如某些需要立刻拿到上一步结果才能组下一批的情形，
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">scheduler.py ::Scheduler.event_loop_overlap</span><span class="ln">错位流水 / 慢一拍</span></div>
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>代价是慢一拍</strong>：你永远比实时<strong>晚一步</strong>处理结果（第 8 课的吞吐/延迟权衡），外加跨拍状态管理更难——下一批在当前结果未出时就搭好，SGLang 用 <span class="mono">batch_overlap</span> 的 future/event 扛依赖。</li>
...
<div class="card macro">
  ...
  which SGLang handles for you via the future/event machinery in <span class="mono">batch_overlap</span>. This is v0.4's "zero-overhead batch scheduler"; it pairs perfectly with continuous batching (Lesson 5): <strong>the batch is always full, and the scheduling that keeps it full costs nothing</strong>.
</div>
...
which SGLang threads through the future/event objects in the <span class="mono">batch_overlap</span> directory to keep correct. Some cases (where you must have the previous result before forming the next,
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">scheduler.py ::Scheduler.event_loop_overlap</span><span class="ln">staggered / one beat behind</span></div>
<pre><span class="kw">def</span> event_loop_overlap(self):
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>The cost is one beat</strong>: you always process results <strong>one step late</strong> (Lesson 8's throughput/latency trade-off), plus harder cross-beat state—the next batch is built before the current result exists, so SGLang carries dependencies via <span class="mono">batch_overlap</span>'s future/event objects.</li>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">schedule_policy.py ::PrefillAdder.add_one_req</span><span class="ln">切块判断</span></div>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">schedule_policy.py ::PrefillAdder.add_one_req</span><span class="ln">chunk decision</span></div>
