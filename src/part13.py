...
<div class="fig">
  ...
  <div class="figcap"><b>图 1 · CPU 气泡藏到前向之后</b> — 串行时 GPU 在 CPU 调度与处理期间空等（气泡）；重叠时 CPU 与 GPU 前向并行，GPU 背靠背连跑，气泡消失。</div>
</div>
...
<div class="fig">
  ...
  <div class="figcap"><b>图 2 · 重叠流水线</b> — 构建、前向、处理三条泳道各错开一拍；进入稳定态后（t4），同一拍里 CPU 构建批 N+1、GPU 跑前向 N、CPU 处理批 N-1，三段始终并行。</div>
</div>
...
<div class="fig">
  ...
  <div class="figcap"><b>Fig 1 · CPU bubbles hidden behind the forward</b> — serial: the GPU idles during CPU schedule and processing (the bubble); overlap: the CPU runs concurrently with the GPU forward, the GPU runs back-to-back, the bubbles vanish.</div>
</div>
...
<div class="fig">
  ...
  <div class="figcap"><b>Fig 2 · the overlap pipeline</b> — build, forward, and process lanes are each staggered one step apart; once in steady state (t4), a single beat has the CPU build batch N+1, the GPU run forward N, and the CPU process batch N-1—all three always in parallel.</div>
</div>
