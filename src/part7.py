...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">python/sglang/srt/mem_cache/radix_cache.py ::RadixCache._match_prefix_helper</span><span class="ln">下行匹配与按需分裂</span></div>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">python/sglang/srt/mem_cache/radix_cache.py ::RadixCache._match_prefix_helper</span><span class="ln">walk-down match and on-demand split</span></div>
  <pre><span class="kw">def</span> _match_prefix_helper(self, node, key):
...
<div class="fig">
  <svg viewBox="0 0 800 300" role="img" aria-label="左边 ReqToTokenPool 是索引账本，按请求和位置查出 token 槽位号；右边 TokenToKVPool 按槽位号存每层的物理 K/V；一个箭头从左表的某个槽位号指向右边数据缓冲里的同一个槽位，体现两级间接寻址">
    ...
    <path d="M 316 80 L 618 80 L 618 84" style="fill:none;stroke:var(--accent);stroke-width:2;stroke-dasharray:5 4"/>
    <polygon points="611,82 625,82 618,91" style="fill:var(--accent)"/>
    <text x="450" y="66" text-anchor="middle" style="fill:var(--accent-ink);font-size:11px">槽位号 #12 → 指向数据（间接寻址）</text>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">python/sglang/srt/mem_cache/memory_pool.py ::ReqToTokenPool</span><span class="ln">私有账本 + 每层 K/V 仓库</span></div>
...
<div class="fig">
  <svg viewBox="0 0 800 300" role="img" aria-label="Left, ReqToTokenPool is the index ledger that maps a request row and position to a token slot number; right, TokenToKVPool stores the per-layer physical K/V indexed by those slot numbers; an arrow runs from a slot number in the left table into the same slot in the right data buffer, showing two-hop indirection">
    ...
    <path d="M 316 80 L 618 80 L 618 84" style="fill:none;stroke:var(--accent);stroke-width:2;stroke-dasharray:5 4"/>
    <polygon points="611,82 625,82 618,91" style="fill:var(--accent)"/>
    <text x="450" y="66" text-anchor="middle" style="fill:var(--accent-ink);font-size:11px">slot id #12 → points at data (indirection)</text>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">python/sglang/srt/mem_cache/memory_pool.py ::ReqToTokenPool</span><span class="ln">private ledger + per-layer K/V warehouse</span></div>
  <pre><span class="kw">class</span> ReqToTokenPool:
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">python/sglang/srt/mem_cache/hiradix_cache.py ::HiRadixCache</span><span class="ln">写回下沉：GPU → host</span></div>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">python/sglang/srt/mem_cache/hiradix_cache.py ::HiRadixCache</span><span class="ln">writeback down: GPU → host</span></div>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">python/sglang/srt/mem_cache/evict_policy.py ::LRUStrategy</span><span class="ln">优先级 = 最近访问时间</span></div>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">python/sglang/srt/mem_cache/evict_policy.py ::LRUStrategy</span><span class="ln">priority = last access time</span></div>
  <pre><span class="kw">class</span> <span class="st">EvictionStrategy</span>(ABC):
