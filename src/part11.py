...
<div class="cols"><div class="col"><strong>VLM 专属（新增的两处接缝）</strong><ul><li>处理器：像素 / 音频 → 张量 + 插占位符</li><li>编码器：ViT / 音频编码器算媒体嵌入</li><li>缝合：在占位符位置散射嵌入</li></ul></div><div class="col"><strong>完全复用（一行不改）</strong><ul><li>调度器：连续批处理（第18课）</li><li>分页 KV 缓存：分页存取（第30课）</li><li>RadixAttention：前缀复用（第33课）</li><li>采样器：从 logits 采样（第28课）</li></ul></div></div>
...
<li>缝合后序列只是一行嵌入，所以<strong>调度器（第18课）/ 分页 KV（第30课）/ RadixAttention（第33课）/ 采样器（第28课）全部复用</strong>。</li>
...
<div class="cols"><div class="col"><strong>VLM-specific (the two new seams)</strong><ul><li>processor: pixels / audio → tensors + insert placeholders</li><li>encoder: ViT / audio encoder computes media embeddings</li><li>splice: scatter embeddings at placeholder positions</li></ul></div><div class="col"><strong>Fully reused (not a line changed)</strong><ul><li>scheduler: continuous batching (L18)</li><li>paged KV cache: paged storage (L30)</li><li>RadixAttention: prefix reuse (L33)</li><li>sampler: sample from logits (L28)</li></ul></div></div>
...
<li>After the splice the sequence is just a row of embeddings, so the <strong>scheduler (L18) / paged KV (L30) / RadixAttention (L33) / sampler (L28) are all reused</strong>.</li>
...
<h2>批处理才是真正的难点</h2>
<p>如果一个批次里所有请求都用同一个适配器，那事情很简单：把 ΔW 算一遍，加到整批上即可。但现实是——同一批里的请求往往<strong>各要各的适配器</strong>：req1 要 adapterA、req2 要 adapterB、req3 又要 adapterA、req4 要 adapterC……如果按适配器一个一个循环处理，就退化成了多次小 GEMM，把连续批处理（<strong>第6课</strong>）辛辛苦苦攒起来的吞吐又拆散了。</p>
<p>SGLang 的关键函数是 <span class="mono">LoRAManager.prepare_lora_batch(forward_batch)</span>。它接过当前的 <span class="mono">ForwardBatch</span>（<strong>第19课</strong>），<strong>逐请求</strong>看清每一行需要哪个适配器，把这些适配器的权重在内存里<strong>摆放/分段（stage &amp; segment）</strong>好，使得后续可以用一次<strong>分段 / 分组 GEMM（segmented / grouped GEMM）</strong>，在<span class="mono">一次</span>内核启动里就把每个请求<strong>各自的</strong> ΔW 都应用上去，而不是为每个适配器单独跑一趟。</p>
...
<p>把视角拉回到一次真实请求：客户端发来一段提示词，并在参数里写明"我要用 <span class="mono">sql-expert</span> 这个适配器"。请求进入调度器排队、与其他请求一起被组成连续批（第6课）。前向开始前，<span class="mono">prepare_lora_batch</span> 扫一遍这批里每个请求的适配器名，发现里面混着 <span class="mono">sql-expert</span>、<span class="mono">med-qa</span>、还有几条不带适配器的纯基座请求——它把不带适配器的当作"零增量"统一处理，把带适配器的按名分段。前向跑到注入了 LoRA 的那些线性层时，先算基座的 <span class="mono">W·x</span>，再用分组 GEMM 把每段各自的 <span class="mono">B·A·x</span> 加上去，得到 <span class="mono">(W + B·A)·x</span>。从采样、KV 缓存到输出，后面的一切都和普通文本推理没有区别。</p>
...
<li>关联：<strong>第19课</strong> ForwardBatch、<strong>第25课</strong> 基座权重、<strong>第6课</strong> 连续批处理；前瞻 <strong>第51课</strong>（RL 也换权重，但换的是<span class="mono">整个</span>模型）。</li>
</ul>
</div>
""", "en": r"""
...
<p>If every request in a batch used the same adapter, it would be easy: compute ΔW once and add it to the whole batch. But in reality requests in one batch often <strong>each want a different adapter</strong>: req1→adapterA, req2→adapterB, req3→adapterA, req4→adapterC… If you loop over adapters one by one, you degrade into many small GEMMs and shred the throughput that continuous batching (<strong>Lesson 6</strong>) worked so hard to accumulate.</p>
<p>SGLang's key function is <span class="mono">LoRAManager.prepare_lora_batch(forward_batch)</span>. It takes the current <span class="mono">ForwardBatch</span> (<strong>Lesson 19</strong>), looks <strong>per request</strong> at which adapter each row needs, and <strong>stages and segments</strong> those adapter weights in memory so that a single <strong>segmented / grouped GEMM</strong> can apply <strong>each request's own</strong> ΔW in <span class="mono">one</span> kernel launch, rather than running a separate pass per adapter.</p>
...
<li>Ties: <strong>Lesson 19</strong> ForwardBatch, <strong>Lesson 25</strong> base weights, <strong>Lesson 6</strong> continuous batching; forward-ref <strong>Lesson 51</strong> (RL also swaps weights, but the <span class="mono">whole</span> model).</li>
</ul>
</div>
"""}
LESSON_51 = {"zh": r"""
