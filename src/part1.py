...
<p>给个具体的数量级：光是把模型装进显存就不便宜——<strong>一个 7B 模型的权重就有约 14 GB（fp16）</strong>，
再加上每条请求各自的 KV 缓存，显存很快就吃紧。而你要写的不过是一行
<span class="mono">engine.generate("中国的首都是", {"max_new_tokens": 64})</span>，
引擎在背后替你扛下批处理、缓存与显存这些重活。</p>
...
<div class="fig">
  <svg viewBox="0 0 720 320" role="img" aria-label="对比：裸模型 model.generate() 一次只跑一条、GPU 常空转，而 SGLang 引擎用调度器、KV 缓存与连续批处理把多请求打包、GPU 持续满载">
    <line x1="360" y1="24" x2="360" y2="296" style="stroke:var(--line);stroke-width:1.5;stroke-dasharray:5 5"/>
    <text x="24" y="38" style="font-weight:700;fill:var(--muted)">裸模型：model.generate()</text>
    <rect x="24" y="54" width="150" height="32" rx="6" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    <text x="99" y="75" text-anchor="middle">单条请求</text>
    <text x="24" y="116" style="fill:var(--muted);font-size:12px">GPU 时间线</text>
    <rect x="24" y="126" width="312" height="30" rx="6" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    <rect x="30" y="130" width="46" height="22" rx="4" style="fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.5"/>
    <text x="53" y="146" text-anchor="middle" class="mono" style="font-size:11px">忙</text>
    <text x="206" y="146" text-anchor="middle" style="fill:var(--faint);font-size:12px">空转 · 空转 · 空转</text>
    <rect x="24" y="186" width="312" height="34" rx="6" style="fill:var(--red-soft);stroke:var(--red);stroke-width:1.5"/>
    <text x="180" y="208" text-anchor="middle" style="fill:var(--red);font-weight:700">一次只服务一条，GPU 多在空转 → 贵</text>
    <text x="384" y="38" style="font-weight:700;fill:var(--accent-ink)">SGLang 引擎</text>
    <rect x="384" y="54" width="312" height="32" rx="6" style="fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.5"/>
    <text x="540" y="75" text-anchor="middle" class="mono" style="font-size:12px">Scheduler · KV 缓存 · 连续批处理</text>
    <text x="384" y="116" style="fill:var(--muted);font-size:12px">GPU 时间线（喂满）</text>
    <rect x="384" y="126" width="312" height="30" rx="6" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    <rect x="388" y="130" width="74" height="22" rx="4" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="425" y="146" text-anchor="middle" class="mono" style="font-size:11px">req1</text>
    <rect x="464" y="130" width="74" height="22" rx="4" style="fill:var(--amber-soft);stroke:var(--amber);stroke-width:1.5"/>
    <text x="501" y="146" text-anchor="middle" class="mono" style="font-size:11px">req2</text>
    <rect x="540" y="130" width="74" height="22" rx="4" style="fill:var(--purple-soft);stroke:var(--purple);stroke-width:1.5"/>
    <text x="577" y="146" text-anchor="middle" class="mono" style="font-size:11px">req3</text>
    <rect x="616" y="130" width="76" height="22" rx="4" style="fill:var(--teal-soft);stroke:var(--teal);stroke-width:1.5"/>
    <text x="654" y="146" text-anchor="middle" class="mono" style="font-size:11px">req4</text>
    <rect x="384" y="186" width="312" height="34" rx="6" style="fill:var(--teal-soft);stroke:var(--teal);stroke-width:1.5"/>
    <text x="540" y="208" text-anchor="middle" style="fill:var(--teal);font-weight:700">多请求打包，GPU 持续满载 → 省</text>
  </svg>
  <div class="figcap"><b>图 1 · 服务引擎 vs 裸模型</b> — 裸模型一次只跑一条、GPU 常空转；SGLang 引擎用调度器、KV 缓存与连续批处理把多请求打包，让同一张 GPU 持续满载。</div>
</div>
...
<div class="fig">
  <svg viewBox="0 0 760 240" role="img" aria-label="自回归循环：提示词“中国的首都是”每步只前向一次、只生成一个 token（北、京…），再喂回去继续直到结束">
    <path d="M 542 96 C 542 26, 239 26, 239 92" style="fill:none;stroke:var(--blue);stroke-width:1.5;stroke-dasharray:5 5"/>
    <polygon points="239,100 232,86 246,86" style="fill:var(--blue)"/>
    <text x="390" y="30" text-anchor="middle" style="fill:var(--blue);font-size:12px">每步只生成 1 个 token，再喂回去</text>
    <rect x="20" y="100" width="150" height="46" rx="8" style="fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.5"/>
    <text x="95" y="128" text-anchor="middle">提示词：中国的首都是</text>
    <text x="180" y="129" text-anchor="middle" style="fill:var(--muted)">→</text>
    <rect x="196" y="100" width="86" height="46" rx="8" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    <text x="239" y="128" text-anchor="middle" class="mono">前向</text>
    <text x="292" y="129" text-anchor="middle" style="fill:var(--muted)">→</text>
    <rect x="308" y="100" width="64" height="46" rx="8" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="340" y="128" text-anchor="middle">北</text>
    <text x="384" y="129" text-anchor="middle" style="fill:var(--muted)">→</text>
    <rect x="398" y="100" width="86" height="46" rx="8" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    <text x="441" y="128" text-anchor="middle" class="mono">前向</text>
    <text x="494" y="129" text-anchor="middle" style="fill:var(--muted)">→</text>
    <rect x="510" y="100" width="64" height="46" rx="8" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="542" y="128" text-anchor="middle">京</text>
    <text x="586" y="129" text-anchor="middle" style="fill:var(--muted)">→</text>
    <rect x="602" y="100" width="138" height="46" rx="8" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    <text x="671" y="128" text-anchor="middle">… tokenₙ（直到结束）</text>
  </svg>
  <div class="figcap"><b>图 2 · 自回归：一个提示词 → 一串 token</b> — 每一步只前向一次、只生成 1 个 token（“中国的首都是”→“北”→“京”…），再把它接回去继续，直到结束。</div>
</div>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">python/sglang/srt/entrypoints/engine.py ::Engine.generate</span><span class="ln">离线进程内的生成 API：传 prompt + 采样参数，可流式</span></div>
  <pre><span class="kw">class</span> Engine:
    <span class="kw">def</span> generate(
        self,
        prompt=<span class="kw">None</span>,               <span class="cm"># 单条 prompt 或一批 prompt</span>
        sampling_params=<span class="kw">None</span>,      <span class="cm"># temperature / top_p / max_new_tokens ...</span>
        image_data=<span class="kw">None</span>,           <span class="cm"># 可选的多模态输入</span>
        return_logprob=<span class="kw">False</span>,
        stream=<span class="kw">False</span>,              <span class="cm"># 边生成边流式返回 token</span>
    ):
        <span class="cm"># 离线、进程内（不走 HTTP）。返回 dict；当 stream=True 时返回迭代器</span>
        ...</pre>
</div>
...
<p>To put a number on it: just fitting the model into GPU memory is not cheap —
<strong>a 7B model's weights alone are ~14 GB in fp16</strong>, and each request's KV cache piles on top,
so memory gets tight fast. Yet all you write is one line,
<span class="mono">engine.generate("The capital of France is", {"max_new_tokens": 64})</span>,
while the engine shoulders the batching, caching and memory work behind it.</p>
...
<div class="fig">
  <svg viewBox="0 0 720 320" role="img" aria-label="Contrast: raw model.generate() serves one request at a time and idles the GPU, while the SGLang engine packs many requests with a scheduler, KV cache and continuous batching to keep the GPU full">
    <line x1="360" y1="24" x2="360" y2="296" style="stroke:var(--line);stroke-width:1.5;stroke-dasharray:5 5"/>
    <text x="24" y="38" style="font-weight:700;fill:var(--muted)">Raw model: model.generate()</text>
    <rect x="24" y="54" width="150" height="32" rx="6" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    <text x="99" y="75" text-anchor="middle">one request</text>
    <text x="24" y="116" style="fill:var(--muted);font-size:12px">GPU timeline</text>
    <rect x="24" y="126" width="312" height="30" rx="6" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    <rect x="30" y="130" width="46" height="22" rx="4" style="fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.5"/>
    <text x="53" y="146" text-anchor="middle" class="mono" style="font-size:11px">busy</text>
    <text x="206" y="146" text-anchor="middle" style="fill:var(--faint);font-size:12px">idle · idle · idle</text>
    <rect x="24" y="186" width="312" height="34" rx="6" style="fill:var(--red-soft);stroke:var(--red);stroke-width:1.5"/>
    <text x="180" y="208" text-anchor="middle" style="fill:var(--red);font-weight:700">one at a time; GPU mostly idle → costly</text>
    <text x="384" y="38" style="font-weight:700;fill:var(--accent-ink)">SGLang Engine</text>
    <rect x="384" y="54" width="312" height="32" rx="6" style="fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.5"/>
    <text x="540" y="75" text-anchor="middle" class="mono" style="font-size:12px">Scheduler · KV cache · continuous batching</text>
    <text x="384" y="116" style="fill:var(--muted);font-size:12px">GPU timeline (packed)</text>
    <rect x="384" y="126" width="312" height="30" rx="6" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    <rect x="388" y="130" width="74" height="22" rx="4" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="425" y="146" text-anchor="middle" class="mono" style="font-size:11px">req1</text>
    <rect x="464" y="130" width="74" height="22" rx="4" style="fill:var(--amber-soft);stroke:var(--amber);stroke-width:1.5"/>
    ...
    <text x="654" y="146" text-anchor="middle" class="mono" style="font-size:11px">req4</text>
    <rect x="384" y="186" width="312" height="34" rx="6" style="fill:var(--teal-soft);stroke:var(--teal);stroke-width:1.5"/>
    <text x="540" y="208" text-anchor="middle" style="fill:var(--teal);font-weight:700;font-size:13px">many requests packed; GPU stays full → cheap</text>
  </svg>
  <div class="figcap"><b>Fig 1 · Serving engine vs raw model</b> — the raw model runs one request at a time and the GPU often idles; the SGLang engine packs many requests via a scheduler, KV cache and continuous batching so one GPU stays full.</div>
</div>
...
<div class="fig">
  <svg viewBox="0 0 760 240" role="img" aria-label="Autoregressive loop: the prompt 'The capital of France is' runs one forward per step, emits a single token (Paris, .) and feeds it back to continue until the end">
    <path d="M 542 96 C 542 26, 239 26, 239 92" style="fill:none;stroke:var(--blue);stroke-width:1.5;stroke-dasharray:5 5"/>
    <polygon points="239,100 232,86 246,86" style="fill:var(--blue)"/>
    <text x="390" y="30" text-anchor="middle" style="fill:var(--blue);font-size:12px">each step emits 1 token, fed back</text>
    <rect x="20" y="100" width="150" height="46" rx="8" style="fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.5"/>
    <text x="95" y="128" text-anchor="middle">Prompt: …France is</text>
    <text x="180" y="129" text-anchor="middle" style="fill:var(--muted)">→</text>
    <rect x="196" y="100" width="86" height="46" rx="8" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    <text x="239" y="128" text-anchor="middle" class="mono">forward</text>
    <text x="292" y="129" text-anchor="middle" style="fill:var(--muted)">→</text>
    <rect x="308" y="100" width="64" height="46" rx="8" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="340" y="128" text-anchor="middle">Paris</text>
    <text x="384" y="129" text-anchor="middle" style="fill:var(--muted)">→</text>
    <rect x="398" y="100" width="86" height="46" rx="8" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    <text x="441" y="128" text-anchor="middle" class="mono">forward</text>
    <text x="494" y="129" text-anchor="middle" style="fill:var(--muted)">→</text>
    <rect x="510" y="100" width="64" height="46" rx="8" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="542" y="128" text-anchor="middle">.</text>
    <text x="586" y="129" text-anchor="middle" style="fill:var(--muted)">→</text>
    <rect x="602" y="100" width="138" height="46" rx="8" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    <text x="671" y="128" text-anchor="middle">… tokenₙ (until end)</text>
  </svg>
  <div class="figcap"><b>Fig 2 · Autoregressive: one prompt → a stream of tokens</b> — each step runs one forward and emits just 1 token ("The capital of France is" → "Paris" → "." …), then feeds it back to continue until the end.</div>
</div>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">python/sglang/srt/entrypoints/engine.py ::Engine.generate</span><span class="ln">the in-process offline generate API: pass a prompt + sampling params, can stream</span></div>
  <pre><span class="kw">class</span> Engine:
    <span class="kw">def</span> generate(
        self,
        prompt=<span class="kw">None</span>,               <span class="cm"># a single prompt or a batch of prompts</span>
        sampling_params=<span class="kw">None</span>,      <span class="cm"># temperature / top_p / max_new_tokens ...</span>
        image_data=<span class="kw">None</span>,           <span class="cm"># optional multimodal input</span>
        return_logprob=<span class="kw">False</span>,
        stream=<span class="kw">False</span>,              <span class="cm"># stream tokens as they are produced</span>
    ):
        <span class="cm"># offline, in-process (NO HTTP). Returns a dict, or an iterator if stream=True</span>
        ...</pre>
</div>
...
<div class="card analogy">
  ...
  怎么排队，但调度长自己<strong>不动手干活</strong>；真正流水线上拧螺丝的，是车间里的<strong>技工（TpModelWorker / ModelRunner）</strong>，
...
<div class="fig">
  <svg viewBox="0 0 760 330" role="img" aria-label="代码仓库地图：python/sglang 切成两半，左边 lang/ 是前端 DSL（ir/interpreter/tracer），右边 srt/ 是运行时引擎（entrypoints/managers/model_executor/layers/mem_cache）">
    <rect x="300" y="14" width="160" height="34" rx="8" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    <text x="380" y="36" text-anchor="middle" class="mono" style="font-size:13px;font-weight:700">python/sglang/</text>
    <line x1="380" y1="48" x2="190" y2="80" style="stroke:var(--line);stroke-width:1.5"/>
    <line x1="380" y1="48" x2="570" y2="80" style="stroke:var(--line);stroke-width:1.5"/>
    <rect x="40" y="80" width="300" height="36" rx="8" style="fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.5"/>
    <text x="190" y="103" text-anchor="middle" style="fill:var(--accent-ink);font-weight:700">lang/ · 前端 DSL（怎么表达）</text>
    <rect x="420" y="80" width="300" height="36" rx="8" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="570" y="103" text-anchor="middle" style="fill:var(--blue);font-weight:700">srt/ · 运行时引擎（怎么执行）</text>
    <rect x="40" y="130" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--accent);stroke-width:1.5"/>
    <text x="54" y="151" class="mono" style="font-size:12px">ir.py</text>
    <text x="326" y="151" text-anchor="end" style="fill:var(--muted);font-size:11px">中间表示（IR）</text>
    <rect x="40" y="168" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--accent);stroke-width:1.5"/>
    <text x="54" y="189" class="mono" style="font-size:12px">interpreter.py</text>
    <text x="326" y="189" text-anchor="end" style="fill:var(--muted);font-size:11px">解释执行</text>
    <rect x="40" y="206" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--accent);stroke-width:1.5"/>
    <text x="54" y="227" class="mono" style="font-size:12px">tracer.py</text>
    <text x="326" y="227" text-anchor="end" style="fill:var(--muted);font-size:11px">编译 / 追踪</text>
    <rect x="420" y="130" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--blue);stroke-width:1.5"/>
    <text x="434" y="151" class="mono" style="font-size:12px">entrypoints/</text>
    <text x="706" y="151" text-anchor="end" style="fill:var(--muted);font-size:11px">HTTP / Engine 入口</text>
    <rect x="420" y="168" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--blue);stroke-width:1.5"/>
    <text x="434" y="189" class="mono" style="font-size:12px">managers/</text>
    <text x="706" y="189" text-anchor="end" style="fill:var(--muted);font-size:11px">分词 · Scheduler 大脑</text>
    <rect x="420" y="206" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--blue);stroke-width:1.5"/>
    <text x="434" y="227" class="mono" style="font-size:12px">model_executor/</text>
    <text x="706" y="227" text-anchor="end" style="fill:var(--muted);font-size:11px">GPU 模型前向</text>
    <rect x="420" y="244" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--blue);stroke-width:1.5"/>
    <text x="434" y="265" class="mono" style="font-size:12px">layers/</text>
    <text x="706" y="265" text-anchor="end" style="fill:var(--muted);font-size:11px">注意力等算子</text>
    <rect x="420" y="282" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--blue);stroke-width:1.5"/>
    <text x="434" y="303" class="mono" style="font-size:12px">mem_cache/</text>
    <text x="706" y="303" text-anchor="end" style="fill:var(--muted);font-size:11px">KV 缓存 · RadixAttention</text>
  </svg>
  <div class="figcap"><b>图 1 · 代码仓库地图</b> — <span class="mono">python/sglang/</span> 第一刀切两半：左边 <span class="mono">lang/</span> 是前端 DSL（ir / interpreter / tracer），右边 <span class="mono">srt/</span> 是运行时引擎，内部又分 entrypoints / managers / model_executor / layers / mem_cache 等子包。</div>
</div>

<p>把这张地图落到具体文件上会更踏实：<span class="mono">srt/</span> 下有 <span class="mono">entrypoints</span> / <span class="mono">managers</span> / <span class="mono">model_executor</span> / <span class="mono">layers</span> / <span class="mono">mem_cache</span> 等子包，而其中真正的<strong>引擎心脏</strong>是 <span class="mono">python/sglang/srt/managers/scheduler.py</span>——后面绝大多数课，都是围着这颗心脏一圈圈展开的。</p>
...
<div class="flow">
  ...
  <div class="arrow">→</div>
  <div class="node"><div class="nt">Scheduler + TpModelWorker</div><div class="nd">子进程 · 每 TP rank 一个 · GPU 前向</div></div>
  <div class="arrow">→</div>
...
<div class="fig">
  <svg viewBox="0 0 760 230" role="img" aria-label="三进程拓扑：TokenizerManager（主进程）经 ZMQ/IPC 把请求发给 Scheduler（子进程，做 GPU 前向），Scheduler 把输出 token 发给 DetokenizerManager（子进程），反分词结果再流式回传 TokenizerManager">
    <text x="380" y="32" text-anchor="middle" style="fill:var(--amber);font-weight:600;font-size:11px">★ 三进程中只有 Scheduler 握着 GPU，两端 Manager 只做翻译</text>
    <rect x="24" y="64" width="190" height="72" rx="10" style="fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.5"/>
    <text x="119" y="96" text-anchor="middle" style="fill:var(--accent-ink);font-weight:700;font-size:13px">TokenizerManager</text>
    <text x="119" y="118" text-anchor="middle" style="fill:var(--muted);font-size:12px">主进程 · 分词收发</text>
    <rect x="285" y="64" width="190" height="72" rx="10" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="380" y="96" text-anchor="middle" style="fill:var(--blue);font-weight:700;font-size:13px">Scheduler (+TpModelWorker)</text>
    <text x="380" y="118" text-anchor="middle" style="fill:var(--muted);font-size:12px">子进程 · GPU 模型前向</text>
    <rect x="546" y="64" width="190" height="72" rx="10" style="fill:var(--purple-soft);stroke:var(--purple);stroke-width:1.5"/>
    <text x="641" y="96" text-anchor="middle" style="fill:var(--purple);font-weight:700;font-size:12px">DetokenizerManager</text>
    <text x="641" y="118" text-anchor="middle" style="fill:var(--muted);font-size:12px">子进程 · 反分词</text>
    <text x="249" y="84" text-anchor="middle" style="fill:var(--muted);font-size:10px">请求 token (ZMQ)</text>
    <line x1="214" y1="100" x2="279" y2="100" style="stroke:var(--muted);stroke-width:1.5"/>
    <polygon points="279,95 279,105 285,100" style="fill:var(--muted)"/>
    <text x="510" y="84" text-anchor="middle" style="fill:var(--muted);font-size:10px">输出 token (ZMQ)</text>
    <line x1="475" y1="100" x2="540" y2="100" style="stroke:var(--muted);stroke-width:1.5"/>
    <polygon points="540,95 540,105 546,100" style="fill:var(--muted)"/>
    <text x="380" y="186" text-anchor="middle" style="fill:var(--teal);font-size:11px">反分词结果回传主进程，流式返回给用户</text>
    <path d="M 641 136 V 200 H 119 V 142" style="fill:none;stroke:var(--teal);stroke-width:1.5;stroke-dasharray:5 5"/>
    <polygon points="113,148 125,148 119,140" style="fill:var(--teal)"/>
  </svg>
  <div class="figcap"><b>图 2 · 三进程拓扑</b> — <span class="mono">TokenizerManager</span>（主进程）经 ZMQ/IPC 把请求发给 <span class="mono">Scheduler</span>（子进程，独占 GPU 做前向），再交给 <span class="mono">DetokenizerManager</span>（子进程）反分词；结果沿底部回路流式返回主进程。只有 Scheduler 握着 GPU。</div>
</div>
...
<span class="inline">TpModelWorker</span> 与 <span class="inline">ModelRunner</span>，真正占着一张 GPU 干前向；<strong>DetokenizerManager 又是另一个子进程</strong>。
...
<div class="codefile">
  ...
  <pre><span class="kw">def</span> <span class="fn">_launch_subprocesses</span>(cls, server_args, ...):
    ...
    <span class="cm"># 2) Scheduler 子进程：每个 TP rank 一个，持有 TpModelWorker/ModelRunner，做 GPU 前向</span>
...
<p>真正动手前，最值得先翻一页的是<strong>所有开关的总闸</strong>——<span class="mono">srt/server_args.py</span> 里的 <span class="inline">ServerArgs</span>。
它是一个中心化的 dataclass：你在命令行里敲的每一个 <span class="mono">--xxx</span> flag，几乎都对应它的一个字段。读懂它，就读懂了这台引擎对外暴露的全部旋钮：</p>

<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">python/sglang/srt/server_args.py ::ServerArgs</span><span class="ln">一个中心配置：每个字段自动映射成一个 --kebab CLI flag</span></div>
  <pre><span class="kw">@dataclasses.dataclass</span>
<span class="kw">class</span> <span class="fn">ServerArgs</span>:
    <span class="cm"># 一个中心配置；每个字段自动映射成一个 --kebab-case 命令行 flag</span>
    model_path: str                    <span class="cm"># --model-path：要服务的模型</span>
    host: str = <span class="st">"127.0.0.1"</span>            <span class="cm"># --host</span>
    tp_size: int = 1                   <span class="cm"># --tp-size：张量并行的 rank 数</span>
    mem_fraction_static = None  <span class="cm"># --mem-fraction-static：静态分配的显存占比</span>
    <span class="cm"># …… 还有几十个字段，每个都是你已经理解的旋钮</span></pre>
</div>
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>三进程</strong>：TokenizerManager（主进程）↔ Scheduler+TpModelWorker（子进程，每 TP rank 一个，握着 GPU）↔ DetokenizerManager（子进程），靠 <strong>ZMQ 的 IPC</strong> 通信。</li>
...
<div class="card analogy">
  ...
  <strong>technicians (TpModelWorker / ModelRunner)</strong> who tend the <strong>expensive machines in the machine room (GPUs and
...
<div class="fig">
  <svg viewBox="0 0 760 330" role="img" aria-label="Repo map: python/sglang split into two halves, lang/ on the left (frontend DSL: ir/interpreter/tracer) and srt/ on the right (runtime: entrypoints/managers/model_executor/layers/mem_cache)">
    <rect x="300" y="14" width="160" height="34" rx="8" style="fill:var(--panel-2);stroke:var(--line);stroke-width:1.5"/>
    <text x="380" y="36" text-anchor="middle" class="mono" style="font-size:13px;font-weight:700">python/sglang/</text>
    <line x1="380" y1="48" x2="190" y2="80" style="stroke:var(--line);stroke-width:1.5"/>
    <line x1="380" y1="48" x2="570" y2="80" style="stroke:var(--line);stroke-width:1.5"/>
    <rect x="40" y="80" width="300" height="36" rx="8" style="fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.5"/>
    <text x="190" y="103" text-anchor="middle" style="fill:var(--accent-ink);font-weight:700">lang/ · frontend DSL (express)</text>
    <rect x="420" y="80" width="300" height="36" rx="8" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="570" y="103" text-anchor="middle" style="fill:var(--blue);font-weight:700">srt/ · runtime engine (execute)</text>
    <rect x="40" y="130" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--accent);stroke-width:1.5"/>
    <text x="54" y="151" class="mono" style="font-size:12px">ir.py</text>
    <text x="326" y="151" text-anchor="end" style="fill:var(--muted);font-size:11px">intermediate representation</text>
    <rect x="40" y="168" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--accent);stroke-width:1.5"/>
    <text x="54" y="189" class="mono" style="font-size:12px">interpreter.py</text>
    <text x="326" y="189" text-anchor="end" style="fill:var(--muted);font-size:11px">interpret &amp; run</text>
    <rect x="40" y="206" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--accent);stroke-width:1.5"/>
    <text x="54" y="227" class="mono" style="font-size:12px">tracer.py</text>
    <text x="326" y="227" text-anchor="end" style="fill:var(--muted);font-size:11px">compile / trace</text>
    <rect x="420" y="130" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--blue);stroke-width:1.5"/>
    <text x="434" y="151" class="mono" style="font-size:12px">entrypoints/</text>
    <text x="706" y="151" text-anchor="end" style="fill:var(--muted);font-size:11px">HTTP / Engine entry</text>
    <rect x="420" y="168" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--blue);stroke-width:1.5"/>
    <text x="434" y="189" class="mono" style="font-size:12px">managers/</text>
    <text x="706" y="189" text-anchor="end" style="fill:var(--muted);font-size:11px">tokenize · Scheduler brain</text>
    <rect x="420" y="206" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--blue);stroke-width:1.5"/>
    <text x="434" y="227" class="mono" style="font-size:12px">model_executor/</text>
    <text x="706" y="227" text-anchor="end" style="fill:var(--muted);font-size:11px">GPU model forward</text>
    <rect x="420" y="244" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--blue);stroke-width:1.5"/>
    <text x="434" y="265" class="mono" style="font-size:12px">layers/</text>
    <text x="706" y="265" text-anchor="end" style="fill:var(--muted);font-size:11px">attention &amp; other ops</text>
    <rect x="420" y="282" width="300" height="34" rx="6" style="fill:var(--panel-2);stroke:var(--blue);stroke-width:1.5"/>
    <text x="434" y="303" class="mono" style="font-size:12px">mem_cache/</text>
    <text x="706" y="303" text-anchor="end" style="fill:var(--muted);font-size:11px">KV cache · RadixAttention</text>
  </svg>
  <div class="figcap"><b>Fig 1 · Repo map</b> — <span class="mono">python/sglang/</span> splits first into two halves: <span class="mono">lang/</span> is the frontend DSL (ir / interpreter / tracer), <span class="mono">srt/</span> is the runtime engine, which in turn splits into entrypoints / managers / model_executor / layers / mem_cache and more.</div>
</div>

<p>Pin the map to concrete files and it sticks: under <span class="mono">srt/</span> sit sub-packages like <span class="mono">entrypoints</span> / <span class="mono">managers</span> / <span class="mono">model_executor</span> / <span class="mono">layers</span> / <span class="mono">mem_cache</span>, and the real <strong>heart of the engine</strong> is <span class="mono">python/sglang/srt/managers/scheduler.py</span> — almost every later lesson circles around that heart.</p>
...
<div class="flow">
  ...
  <div class="arrow">→</div>
  <div class="node"><div class="nt">Scheduler + TpModelWorker</div><div class="nd">subprocess · one per TP rank · GPU forward</div></div>
  <div class="arrow">→</div>
...
<div class="fig">
  <svg viewBox="0 0 760 230" role="img" aria-label="Three-process topology: TokenizerManager (main process) sends requests over ZMQ/IPC to Scheduler (subprocess, runs the GPU forward), which sends output tokens to DetokenizerManager (subprocess); detokenized results stream back to TokenizerManager">
    <text x="380" y="32" text-anchor="middle" style="fill:var(--amber);font-weight:600;font-size:11px">★ Only the Scheduler holds the GPU; the two Managers are just translators</text>
    <rect x="24" y="64" width="190" height="72" rx="10" style="fill:var(--accent-soft);stroke:var(--accent);stroke-width:1.5"/>
    <text x="119" y="96" text-anchor="middle" style="fill:var(--accent-ink);font-weight:700;font-size:13px">TokenizerManager</text>
    <text x="119" y="118" text-anchor="middle" style="fill:var(--muted);font-size:12px">main process · tokenize I/O</text>
    <rect x="285" y="64" width="190" height="72" rx="10" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="380" y="96" text-anchor="middle" style="fill:var(--blue);font-weight:700;font-size:13px">Scheduler (+TpModelWorker)</text>
    <text x="380" y="118" text-anchor="middle" style="fill:var(--muted);font-size:12px">subprocess · GPU forward</text>
    <rect x="546" y="64" width="190" height="72" rx="10" style="fill:var(--purple-soft);stroke:var(--purple);stroke-width:1.5"/>
    <text x="641" y="96" text-anchor="middle" style="fill:var(--purple);font-weight:700;font-size:12px">DetokenizerManager</text>
    <text x="641" y="118" text-anchor="middle" style="fill:var(--muted);font-size:12px">subprocess · detokenize</text>
    <text x="249" y="84" text-anchor="middle" style="fill:var(--muted);font-size:10px">request tokens (ZMQ)</text>
    <line x1="214" y1="100" x2="279" y2="100" style="stroke:var(--muted);stroke-width:1.5"/>
    <polygon points="279,95 279,105 285,100" style="fill:var(--muted)"/>
    <text x="510" y="84" text-anchor="middle" style="fill:var(--muted);font-size:10px">output tokens (ZMQ)</text>
    <line x1="475" y1="100" x2="540" y2="100" style="stroke:var(--muted);stroke-width:1.5"/>
    <polygon points="540,95 540,105 546,100" style="fill:var(--muted)"/>
    <text x="380" y="186" text-anchor="middle" style="fill:var(--teal);font-size:11px">detokenized result streams back to the main process</text>
    <path d="M 641 136 V 200 H 119 V 142" style="fill:none;stroke:var(--teal);stroke-width:1.5;stroke-dasharray:5 5"/>
    <polygon points="113,148 125,148 119,140" style="fill:var(--teal)"/>
  </svg>
  <div class="figcap"><b>Fig 2 · Three-process topology</b> — <span class="mono">TokenizerManager</span> (main process) sends requests over ZMQ/IPC to the <span class="mono">Scheduler</span> (subprocess, which alone runs the GPU forward), then to <span class="mono">DetokenizerManager</span> (subprocess); the result streams back to the main process along the bottom path. Only the Scheduler holds the GPU.</div>
</div>
...
Scheduler process per rank</strong>, each holding its own <span class="inline">TpModelWorker</span> and <span class="inline">ModelRunner</span>
...
<div class="codefile">
  ...
  <pre><span class="kw">def</span> <span class="fn">_launch_subprocesses</span>(cls, server_args, ...):
    ...
    <span class="cm"># 2) Scheduler subprocess: one per TP rank, holds TpModelWorker/ModelRunner, does GPU forward</span>
    scheduler_init_result, scheduler_procs = cls._launch_scheduler_processes(
        server_args, port_args, run_scheduler_process_func)
...
<p>Before you touch anything, the single page most worth reading first is the <strong>master switchboard</strong> —
<span class="inline">ServerArgs</span> in <span class="mono">srt/server_args.py</span>. It is one central dataclass: nearly every
<span class="mono">--xxx</span> flag you type on the command line maps to one of its fields. Read it and you have read every knob this engine exposes:</p>

<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">python/sglang/srt/server_args.py ::ServerArgs</span><span class="ln">the central config: each field auto-maps to a --kebab CLI flag</span></div>
  <pre><span class="kw">@dataclasses.dataclass</span>
<span class="kw">class</span> <span class="fn">ServerArgs</span>:
    <span class="cm"># one central config; each field auto-maps to a --kebab-case CLI flag</span>
    model_path: str                    <span class="cm"># --model-path: the model to serve</span>
    host: str = <span class="st">"127.0.0.1"</span>            <span class="cm"># --host</span>
    tp_size: int = 1                   <span class="cm"># --tp-size: tensor-parallel ranks</span>
    mem_fraction_static = None  <span class="cm"># --mem-fraction-static: static-alloc fraction</span>
    <span class="cm"># ... dozens more fields, each a knob you already understand</span></pre>
</div>
...
<div class="card key">
  ...
  <ul>
    ...
    <li><strong>Three processes</strong>: TokenizerManager (main) ↔ Scheduler+TpModelWorker (subprocess, one per TP rank, holds the GPU) ↔ DetokenizerManager (subprocess), talking over <strong>ZMQ IPC</strong>.</li>
...
<div class="fig">
  <svg viewBox="0 0 820 300" role="img" aria-label="一次 generate 请求的旅程时间轴：HTTP 入口与分词在主进程，组批与前向在 Scheduler 子进程并跑在 GPU，反分词在 Detokenizer 子进程，最后流式返回主进程，跨进程的箭头是一次 ZMQ 传递">
    <text x="22" y="28" style="font-weight:700;fill:var(--muted)">一次请求的旅程（按时间从左到右）</text>
    <rect x="430" y="17" width="12" height="12" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="448" y="27" style="fill:var(--muted);font-size:12px">主进程</text>
    <rect x="520" y="17" width="12" height="12" rx="3" style="fill:var(--purple-soft);stroke:var(--purple);stroke-width:1.5"/>
    <text x="538" y="27" style="fill:var(--muted);font-size:12px">Scheduler 子进程</text>
    <rect x="668" y="17" width="12" height="12" rx="3" style="fill:var(--teal-soft);stroke:var(--teal);stroke-width:1.5"/>
    <text x="686" y="27" style="fill:var(--muted);font-size:12px">Detok 子进程</text>
    <rect x="22" y="80" width="118" height="64" rx="8" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="81" y="108" text-anchor="middle" style="font-size:12px;font-weight:700">HTTP /generate</text>
    <text x="81" y="128" text-anchor="middle" style="fill:var(--muted);font-size:11px">主进程 · 入口</text>
    <rect x="154" y="80" width="118" height="64" rx="8" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="213" y="108" text-anchor="middle" style="font-size:12px;font-weight:700">分词 tokenize</text>
    <text x="213" y="128" text-anchor="middle" style="fill:var(--muted);font-size:11px">TokenizerManager</text>
    <rect x="286" y="80" width="118" height="64" rx="8" style="fill:var(--purple-soft);stroke:var(--purple);stroke-width:1.5"/>
    <text x="345" y="108" text-anchor="middle" style="font-size:12px;font-weight:700">组批 batch</text>
    <text x="345" y="128" text-anchor="middle" style="fill:var(--muted);font-size:11px">Scheduler</text>
    <rect x="418" y="80" width="118" height="64" rx="8" style="fill:var(--purple-soft);stroke:var(--purple);stroke-width:1.5"/>
    <text x="477" y="108" text-anchor="middle" style="font-size:12px;font-weight:700">前向 forward</text>
    <text x="477" y="128" text-anchor="middle" style="fill:var(--purple);font-size:11px">ModelRunner · GPU</text>
    <rect x="550" y="80" width="118" height="64" rx="8" style="fill:var(--teal-soft);stroke:var(--teal);stroke-width:1.5"/>
    <text x="609" y="108" text-anchor="middle" style="font-size:12px;font-weight:700">反分词 detok</text>
    <text x="609" y="128" text-anchor="middle" style="fill:var(--muted);font-size:11px">DetokenizerManager</text>
    <rect x="682" y="80" width="118" height="64" rx="8" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="741" y="108" text-anchor="middle" style="font-size:12px;font-weight:700">流式返回</text>
    <text x="741" y="128" text-anchor="middle" style="fill:var(--muted);font-size:11px">主进程 · SSE</text>
    <text x="147" y="118" text-anchor="middle" style="fill:var(--faint);font-size:16px">→</text>
    <text x="279" y="118" text-anchor="middle" style="fill:var(--faint);font-size:16px">→</text>
    <text x="411" y="118" text-anchor="middle" style="fill:var(--faint);font-size:16px">→</text>
    <text x="543" y="118" text-anchor="middle" style="fill:var(--faint);font-size:16px">→</text>
    <text x="675" y="118" text-anchor="middle" style="fill:var(--faint);font-size:16px">→</text>
    <text x="279" y="70" text-anchor="middle" class="mono" style="fill:var(--accent-ink);font-size:10px">ZMQ</text>
    <text x="543" y="70" text-anchor="middle" class="mono" style="fill:var(--accent-ink);font-size:10px">ZMQ</text>
    <text x="675" y="70" text-anchor="middle" class="mono" style="fill:var(--accent-ink);font-size:10px">ZMQ</text>
    <line x1="22" y1="200" x2="800" y2="200" style="stroke:var(--line);stroke-width:1.5"/>
    <path d="M800 200 l-9 -4 v8 z" style="fill:var(--faint)"/>
    <text x="22" y="222" style="fill:var(--faint);font-size:11px">t0</text>
    <text x="345" y="222" text-anchor="middle" style="fill:var(--faint);font-size:11px">组批 / 前向（循环很多次）</text>
    <text x="760" y="222" style="fill:var(--faint);font-size:11px">时间 →</text>
    <line x1="81" y1="196" x2="81" y2="204" style="stroke:var(--faint);stroke-width:1.5"/>
    <line x1="213" y1="196" x2="213" y2="204" style="stroke:var(--faint);stroke-width:1.5"/>
    <line x1="477" y1="196" x2="477" y2="204" style="stroke:var(--faint);stroke-width:1.5"/>
    <line x1="741" y1="196" x2="741" y2="204" style="stroke:var(--faint);stroke-width:1.5"/>
  </svg>
  <div class="figcap"><b>图 1 · 一次请求的旅程</b> — 从左到右就是一条 <span class="mono">POST /generate</span> 的时间轴：主进程负责 HTTP 入口与分词，Scheduler 子进程在 GPU 上组批与前向（中段会循环很多次），Detokenizer 子进程反分词，最后绕回主进程以 SSE 流式返回；每条跨进程的箭头都是一次 ZMQ 传递。</div>
</div>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">python/sglang/srt/managers/tokenizer_manager.py ::generate_request</span><span class="ln">请求进入引擎的入口：分词 → 经 IPC 交给调度器 → 流式回吐</span></div>
  <pre><span class="kw">async def</span> <span class="fn">generate_request</span>(
    self,
    obj,                  <span class="cm"># 解析好的 GenerateReqInput（prompt、采样参数…）</span>
    request=None,
):
    <span class="cm"># 请求进入引擎的入口：把 prompt 分词，经 IPC(ZMQ) 交给调度器，</span>
    <span class="cm"># 再把生成的 token 流式返回</span>
    ...</pre>
</div>
...
<div class="vflow">
  ...
  <div class="step"><div class="num">3</div><div class="sc"><h4>跑这一批 run_batch</h4><p class="mono">TpModelWorker · ModelRunner</p><p>在 GPU 上做一次模型前向得到 logits，再由 <span class="inline">Sampler</span> 采出每条请求的下一个 token。</p></div></div>
...
<span class="inline">TpModelWorker</span> 里的 <span class="inline">ModelRunner.forward</span>，在显卡上做一次模型前向，算出每条请求“下一个 token”的
...
<p>举个具体的数字感受一下：一段 <strong>128 token</strong> 的提示词，预填充就是<strong>一次</strong>前向把这 128 个 token 并行算完、把它们的 KV 缓存填好；
之后<strong>每生成 1 个 token 又是一次前向</strong>——所以想要 200 个 token 的回答，大致就是 <strong>1 次预填充 + 约 200 次解码前向</strong>，
循环里绝大多数圈都花在“逐 token”的解码上，而不是开头那一次预填充。</p>
...
<div class="fig">
  <svg viewBox="0 0 780 280" role="img" aria-label="预填充与解码随时间对比：预填充是一个宽块，一次并行算完整段提示词，是计算密集；解码是许多细条，每条是一个 token 的一次前向，是访存密集">
    <text x="20" y="28" style="font-weight:700;fill:var(--muted)">预填充 vs 解码（随时间）</text>
    <text x="142" y="60" text-anchor="middle" style="fill:var(--amber);font-size:12px;font-weight:700">prefill（一次）</text>
    <text x="470" y="60" text-anchor="middle" style="fill:var(--blue);font-size:12px;font-weight:700">decode（循环很多次）</text>
    <rect x="56" y="78" width="172" height="92" rx="8" style="fill:var(--amber-soft);stroke:var(--amber);stroke-width:1.5"/>
    <text x="142" y="118" text-anchor="middle" style="font-size:13px;font-weight:700">prefill</text>
    <text x="142" y="138" text-anchor="middle" class="mono" style="fill:var(--muted);font-size:11px">整段提示词</text>
    <rect x="256" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="280" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="304" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="328" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="352" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="376" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="400" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="424" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="448" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="472" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="496" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="520" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="544" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="568" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="592" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="616" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="264" y="102" text-anchor="middle" class="mono" style="fill:var(--blue);font-size:10px">+1</text>
    <text x="312" y="102" text-anchor="middle" class="mono" style="fill:var(--blue);font-size:10px">+1</text>
    <text x="360" y="102" text-anchor="middle" class="mono" style="fill:var(--blue);font-size:10px">+1</text>
    <line x1="40" y1="200" x2="760" y2="200" style="stroke:var(--line);stroke-width:1.5"/>
    <path d="M760 200 l-9 -4 v8 z" style="fill:var(--faint)"/>
    <text x="710" y="222" style="fill:var(--faint);font-size:11px">时间 →</text>
    <text x="142" y="244" text-anchor="middle" style="fill:var(--amber);font-size:11px">一次并行算完整段提示词 · 计算密集</text>
    <text x="470" y="244" text-anchor="middle" style="fill:var(--blue);font-size:11px">逐 token，每步一次前向 · 访存密集</text>
  </svg>
  <div class="figcap"><b>图 2 · 预填充 vs 解码（随时间）</b> — 预填充是<strong>一个宽块</strong>：一次并行算完整段提示词、把 KV 缓存填好，吃算力（计算密集）；解码是后面<strong>许多细条</strong>：每条只算最新的一个 token、做一次前向，瓶颈在反复搬 KV 缓存（访存密集）。一条请求就是“一次宽块 + 很多细条”。</div>
</div>
...
<div class="fig">
  <svg viewBox="0 0 820 300" role="img" aria-label="Timeline of one generate request's journey: HTTP entry and tokenize live in the main process, batch and forward in the Scheduler subprocess on the GPU, detokenize in the Detokenizer subprocess, then stream back to the main process; each cross-process arrow is one ZMQ hop">
    <text x="22" y="28" style="font-weight:700;fill:var(--muted)">A request's journey (left to right over time)</text>
    <rect x="430" y="17" width="12" height="12" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="448" y="27" style="fill:var(--muted);font-size:12px">Main process</text>
    <rect x="540" y="17" width="12" height="12" rx="3" style="fill:var(--purple-soft);stroke:var(--purple);stroke-width:1.5"/>
    <text x="558" y="27" style="fill:var(--muted);font-size:12px">Scheduler subprocess</text>
    <rect x="700" y="17" width="12" height="12" rx="3" style="fill:var(--teal-soft);stroke:var(--teal);stroke-width:1.5"/>
    <text x="718" y="27" style="fill:var(--muted);font-size:12px">Detok subprocess</text>
    <rect x="22" y="80" width="118" height="64" rx="8" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="81" y="108" text-anchor="middle" style="font-size:12px;font-weight:700">HTTP /generate</text>
    <text x="81" y="128" text-anchor="middle" style="fill:var(--muted);font-size:11px">Main · entry</text>
    <rect x="154" y="80" width="118" height="64" rx="8" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="213" y="108" text-anchor="middle" style="font-size:12px;font-weight:700">tokenize</text>
    <text x="213" y="128" text-anchor="middle" style="fill:var(--muted);font-size:11px">TokenizerManager</text>
    <rect x="286" y="80" width="118" height="64" rx="8" style="fill:var(--purple-soft);stroke:var(--purple);stroke-width:1.5"/>
    <text x="345" y="108" text-anchor="middle" style="font-size:12px;font-weight:700">batch</text>
    <text x="345" y="128" text-anchor="middle" style="fill:var(--muted);font-size:11px">Scheduler</text>
    <rect x="418" y="80" width="118" height="64" rx="8" style="fill:var(--purple-soft);stroke:var(--purple);stroke-width:1.5"/>
    <text x="477" y="108" text-anchor="middle" style="font-size:12px;font-weight:700">forward</text>
    <text x="477" y="128" text-anchor="middle" style="fill:var(--purple);font-size:11px">ModelRunner · GPU</text>
    <rect x="550" y="80" width="118" height="64" rx="8" style="fill:var(--teal-soft);stroke:var(--teal);stroke-width:1.5"/>
    <text x="609" y="108" text-anchor="middle" style="font-size:12px;font-weight:700">detokenize</text>
    <text x="609" y="128" text-anchor="middle" style="fill:var(--muted);font-size:11px">DetokenizerManager</text>
    <rect x="682" y="80" width="118" height="64" rx="8" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <text x="741" y="108" text-anchor="middle" style="font-size:12px;font-weight:700">stream back</text>
    <text x="741" y="128" text-anchor="middle" style="fill:var(--muted);font-size:11px">Main · SSE</text>
    <text x="147" y="118" text-anchor="middle" style="fill:var(--faint);font-size:16px">→</text>
    <text x="279" y="118" text-anchor="middle" style="fill:var(--faint);font-size:16px">→</text>
    <text x="411" y="118" text-anchor="middle" style="fill:var(--faint);font-size:16px">→</text>
    <text x="543" y="118" text-anchor="middle" style="fill:var(--faint);font-size:16px">→</text>
    ...
    <path d="M800 200 l-9 -4 v8 z" style="fill:var(--faint)"/>
    <text x="22" y="222" style="fill:var(--faint);font-size:11px">t0</text>
    <text x="345" y="222" text-anchor="middle" style="fill:var(--faint);font-size:11px">batch / forward (loops many times)</text>
    <text x="752" y="222" style="fill:var(--faint);font-size:11px">time →</text>
    <line x1="81" y1="196" x2="81" y2="204" style="stroke:var(--faint);stroke-width:1.5"/>
    <line x1="213" y1="196" x2="213" y2="204" style="stroke:var(--faint);stroke-width:1.5"/>
    <line x1="477" y1="196" x2="477" y2="204" style="stroke:var(--faint);stroke-width:1.5"/>
    <line x1="741" y1="196" x2="741" y2="204" style="stroke:var(--faint);stroke-width:1.5"/>
  </svg>
  <div class="figcap"><b>Fig 1 · A request's journey</b> — left to right is the timeline of one <span class="mono">POST /generate</span>: the main process owns the HTTP entry and tokenize, the Scheduler subprocess batches and forwards on the GPU (the middle loops many times), the Detokenizer subprocess detokenizes, and it finally loops back to the main process to stream out as SSE; every cross-process arrow is one ZMQ hop.</div>
</div>
...
<div class="codefile">
  <div class="cf-head"><span class="dot"></span><span class="path">python/sglang/srt/managers/tokenizer_manager.py ::generate_request</span><span class="ln">the request's entry: tokenize → hand to the scheduler over IPC → stream back</span></div>
  <pre><span class="kw">async def</span> <span class="fn">generate_request</span>(
    self,
    obj,                  <span class="cm"># the parsed GenerateReqInput (prompt, sampling params, ...)</span>
    request=None,
):
    <span class="cm"># the request's entry into the engine: tokenize the prompt, dispatch it to the</span>
    <span class="cm"># Scheduler over IPC (ZMQ), then stream tokens back as they are produced</span>
    ...</pre>
</div>
...
<div class="vflow">
  ...
  <div class="step"><div class="num">3</div><div class="sc"><h4>run_batch</h4><p class="mono">TpModelWorker · ModelRunner</p><p>Run one model forward on the GPU for logits, then <span class="inline">Sampler</span> picks each request's next token.</p></div></div>
...
hands the whole batch to <span class="inline">ModelRunner.forward</span> inside the <span class="inline">TpModelWorker</span>, runs one model
...
<p>Put concrete numbers on it: a <strong>128-token</strong> prompt means prefill is <strong>one</strong> forward pass that computes all 128
tokens in parallel and fills their KV cache; after that, <strong>every single generated token is one more forward pass</strong> — so a
200-token answer is roughly <strong>1 prefill + about 200 decode forwards</strong>, and the vast majority of loop turns are spent on
token-by-token decode, not on that opening prefill.</p>
...
<div class="fig">
  <svg viewBox="0 0 780 280" role="img" aria-label="Prefill vs decode over time: prefill is one wide block that computes the whole prompt in parallel and is compute-bound; decode is many thin bars, each one token's single forward pass and bandwidth-bound">
    <text x="20" y="28" style="font-weight:700;fill:var(--muted)">Prefill vs decode (over time)</text>
    <text x="142" y="60" text-anchor="middle" style="fill:var(--amber);font-size:12px;font-weight:700">prefill (once)</text>
    <text x="470" y="60" text-anchor="middle" style="fill:var(--blue);font-size:12px;font-weight:700">decode (loops many times)</text>
    <rect x="56" y="78" width="172" height="92" rx="8" style="fill:var(--amber-soft);stroke:var(--amber);stroke-width:1.5"/>
    <text x="142" y="118" text-anchor="middle" style="font-size:13px;font-weight:700">prefill</text>
    <text x="142" y="138" text-anchor="middle" class="mono" style="fill:var(--muted);font-size:11px">whole prompt</text>
    <rect x="256" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="280" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="304" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="328" y="110" width="16" height="60" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
