...
<div class="card macro">
  ...
  每个 TP rank 有<strong>一个 ModelRunner</strong>（由 TpWorker 持有），它握着<strong>加载好的模型</strong>（第 25/26 课）和
...
<p>先把调用链补全。第 18 课的调度器主循环每一步都会调 <span class="mono">run_batch</span>，但 run_batch 本身其实很<strong>薄</strong>：它把刚组好的 <span class="mono">ScheduleBatch</span> 交给 <strong>TpWorker</strong>，TpWorker 再原样转手喂给它<strong>独占的那一个 ModelRunner</strong>。这条链路之所以要分这么多层，是因为每一层只盯着一件事——调度器只关心"这一步该算哪一批"，TpWorker 只关心"我这个 rank 的边界与跨卡通信"，ModelRunner 只关心"把张量在这张卡上真正算出来"。层与层之间职责清晰、互不越界，正是这套代码能被上千名贡献者协作维护的原因。</p>
<p>这条链还藏着一个容易忽视却致命的约束：在 8 路 TP 的部署里，8 个 TpWorker 各自的 ModelRunner 必须<strong>严格同步地跑同一批</strong>。它们各算模型的一片（权重按行/列切开，第 25/46 课），层中途要靠 all-reduce / all-gather 把注意力输出和残差拼齐——只要有一个 rank 早走或晚走半步，集合通信就会<strong>整组卡死</strong>。这就是为什么进入前向之前，ScheduleBatch 的内容必须在所有 rank 上<strong>逐字节一致</strong>：同样的 token、同样的形状、同样的 forward_mode。理解了这一点，你就明白第 18 课强调的"调度决策必须确定性、可复现"不是洁癖，而是分布式前向能不死锁的硬前提。</p>

<h2>从 ScheduleBatch 到 ForwardBatch：换一副 GPU 的眼镜</h2>
<p>先厘清一个常被混淆的层级：<strong>TpWorker</strong> 是张量并行里"一个 rank"的封装，它内部<strong>持有一个 ModelRunner</strong>；
所以在 8 路 TP 的部署里，有 8 个 TpWorker、8 个 ModelRunner，各自<strong>只算模型的一片</strong>（权重按列/行切开，第 25/46 课），
...
<div class="card macro">
  ...
  Each TP rank has <strong>one ModelRunner</strong> (owned by the TpWorker); it holds the <strong>loaded model</strong>
...
<strong>TpWorker</strong>, which hands it straight to <strong>the one ModelRunner it owns</strong>. The chain is layered this
deep because each layer watches exactly one thing — the scheduler cares about "which batch to run this step", the TpWorker
...
<p>The chain also hides a subtle but fatal constraint: in an 8-way TP deployment, the 8 TpWorkers' ModelRunners must run
...
<div class="fig">
  <svg viewBox="0 0 800 300" role="img" aria-label="Per-step launch overhead with vs without a graph: without a graph each decode step is many small CPU kernel-launch gaps between GPU kernels and launch overhead dominates; with a graph one submission replays the whole recorded sequence so gaps vanish and the GPU runs back-to-back">
    ...
    <rect x="400" y="78" width="34" height="28" rx="4" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
    <rect x="434" y="78" width="30" height="28" rx="4" style="fill:var(--amber-soft);stroke:var(--amber);stroke-width:1.5"/>
    <text x="476" y="97" style="fill:var(--faint);font-size:12px">… hundreds of kernels × gaps, launch overhead dominates</text>
    <rect x="64" y="124" width="14" height="14" rx="3" style="fill:var(--blue-soft);stroke:var(--blue);stroke-width:1.5"/>
...
<div class="fig">
  <svg viewBox="0 0 800 270" role="img" aria-label="Capture once, replay many: on the left a one-time capture records the kernel sequence for a fixed batch shape into a graph object; on the right many decode steps cheaply replay that recorded graph with new data and the same shape">
    ...
    <rect x="540" y="46" width="244" height="34" rx="7" style="fill:var(--teal-soft);stroke:var(--teal);stroke-width:1.5"/>
    <text x="556" y="68" style="font-size:12px">decode step 1 · one-submission replay (new data · same shape)</text>
    <rect x="540" y="88" width="244" height="34" rx="7" style="fill:var(--teal-soft);stroke:var(--teal);stroke-width:1.5"/>
    <text x="556" y="110" style="font-size:12px">decode step 2 · one-submission replay (new data · same shape)</text>
    <rect x="540" y="130" width="244" height="34" rx="7" style="fill:var(--teal-soft);stroke:var(--teal);stroke-width:1.5"/>
    <text x="556" y="152" style="font-size:12px">decode step 3 · one-submission replay (new data · same shape)</text>
    <text x="540" y="190" style="fill:var(--muted);font-size:12px">… the same graph is replayed thousands of times, near-zero launch overhead</text>
  </svg>
