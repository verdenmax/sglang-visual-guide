...
按配置<strong>实例化模型结构</strong>——这时各层的线性层、注意力、嵌入都建好了，但参数还是<strong>未初始化的占位张量</strong>，几乎不占有效显存。<strong>第二步</strong>，
...
and embeddings are built, but the params are still <strong>uninitialized placeholder tensors</strong>, taking almost no real GPU memory.
...
<div class="card macro">
  ...
  而每条请求的那套旋钮，源头是用户随请求发来的 <span class="mono">SamplingParams</span>（第 53 课）。
</div>
...
<div class="card macro">
  ...
  Each request's knob set comes from the user's <span class="mono">SamplingParams</span> (Lesson 53).
</div>
