...
<div class="card macro">
  ...
  SGLang 提供两条驱动运行时的路径，它们背后是<strong>同一套运行时核心</strong>：
...
<div class="card key">
  ...
  <ul>
    <li><strong>两扇门</strong>：离线 <span class="mono">Engine</span>（纯进程内 Python API，无 HTTP）与在线 server（<span class="mono">launch_server</span> 起的 FastAPI 应用），共用同一套运行时核心（离线 <span class="mono">Engine</span> 继承抽象基类 <span class="mono">EngineBase</span>，在线 server 经 <span class="mono">launch_server</span> 启动同一套运行时）。</li>
...
<div class="card macro">
  ...
  SGLang offers two paths to drive the runtime, both built on the <strong>same runtime core</strong>:
...
<div class="card key">
  ...
  <ul>
    <li><strong>Two doors</strong>: offline <span class="mono">Engine</span> (pure in-process Python API, no HTTP) and online server (a FastAPI app from <span class="mono">launch_server</span>), sharing one runtime core (the offline <span class="mono">Engine</span> subclasses <span class="mono">EngineBase</span>; the online server boots the same runtime via <span class="mono">launch_server</span>).</li>
