...
absolute URL in every page's `<head>`. It is produced by `python3 build_og.py`, which
renders the card — defined as a brand-palette SVG in `build_og.py`'s `card_svg()` — to
PNG via `rsvg-convert` (needs `rsvg-convert` and Noto Sans CJK fonts). The committed
`og-cover.png` is the served asset, so the normal `build.py` stays dependency-free.
...
og-cover.svg    generated share-card SVG (committed; regenerated by build_og.py)
