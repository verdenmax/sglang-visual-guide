...
> **Status:** **in progress (M0 — pipeline + L01 baseline).** The zero-dependency build pipeline and
> project chrome are stood up, with **one lesson (L01 · What is SGLang)** shipped to prove the
> toolchain end-to-end. The guide is built milestone-driven; see `docs/superpowers/plans/` for the
> roadmap toward the full **13 parts / ~63 lessons**.
...
> M0 ships only **L01**; the remaining lessons land milestone by milestone. Per-part counts may shift
> by ±1 as research firms up each milestone (`MAX_LESSON` in `check_html.py` tracks the planned total).
...
src/            generators + tooling (pure Python 3, no dependencies)
  part1.py .. part13.py   lesson content (bilingual), one module per part (only part1 at M0)
...
> **进度：** **进行中（M0 —— 流水线 + L01 基线）。** 已搭好零依赖的构建流水线与项目骨架，并产出
> **第一课（L01 · SGLang 是什么）**以端到端验证工具链。本指南按里程碑分部分构建，路线图见
> `docs/superpowers/plans/`，规划共 **13 个部分 / 约 63 课**。
